...
bool musicxmlscore::newLayout(wxSize sizeClient)
{
	...
	else
	{
		...
		if (bitmapSplash.LoadFile("lilypond.png", wxBITMAP_TYPE_PNG))
			splash = new wxSplashScreen(bitmapSplash, wxSPLASH_CENTRE_ON_PARENT | wxSPLASH_NO_TIMEOUT, 10000,  NULL , -1, wxDefaultPosition, wxDefaultSize,	wxBORDER_SIMPLE | wxSTAY_ON_TOP );
...
bool musicxmlscore::readlilypos()
{
	...
	if (lposnotes.size() == 0)
	{
		wxMessageBox("No note positions found in Lilypond source file expresseur_out.ly", "build score", wxOK | wxICON_ERROR);
		mlog_in("Error no note positions found in Lilypond source file %s", (const char*)(lilysrc.c_str()));
		return false;
	}
	return true;
}
bool musicxmlscore::readlilypdf(uint32_t page, uint32_t xpng, uint32_t ypng)
{
...
#endif
	{
		...
		case 21:
			etat = 0;
			mly.column += 3; // since lilypond 2.25.30
			if (logLilypond)
