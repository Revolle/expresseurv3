...
bool musicxmlscore::newLayout(wxSize sizeClient)
{
	...
	// debug mode :
	fm.SetPath("C:\\Users\\franc\\Documents\\lilypond-2.24.4");
	fm.AppendDir("bin");
...
bool musicxmlscore::readlilypdf(uint32_t page, uint32_t xpng, uint32_t ypng)
{
	...
	wxString s;
	s.Printf(FILE_SCORE_PDF, page);
	fpdf.SetFullName(s);
	FILE* fp = fopen(fpdf.GetFullPath(), "rb");
	if (fp == NULL)
	{
...
bool musicxmlscore::readPos()
{
	...
	while ((ch = getc(fp)) != EOF)
	{
		...
		case 1: 
			...
			// nrpage:x1:y1:x2:y2:
		case 2: 
			if ((ch >= '0') && (ch <= '9')) { nbint *= 10; nbint += (ch - '0'); break; }
			if (ch == ':') { etat++; nrpage = nbint; nbint = 0; break; }
			  etat = 0; break;
		case 3: if ((ch >= '0') && (ch <= '9')) { nbint *= 10; nbint += (ch - '0'); break; }
			  if (ch == ':') { etat++; rect.x = nbint; nbint = 0; break; }
			  etat = 0; break;
		case 4: if ((ch >= '0') && (ch <= '9')) { nbint *= 10; nbint += (ch - '0'); break; }
			  if (ch == ':') { etat++; rect.y = nbint; nbint = 0; break; }
			  etat = 0; break;
		case 5: if ((ch >= '0') && (ch <= '9')) { nbint *= 10; nbint += (ch - '0'); break; }
			  if (ch == ':') { etat++; rect.width = nbint - rect.x ; nbint = 0; break; }
			  etat = 0; break;
		case 6: if ((ch >= '0') && (ch <= '9')) { nbint *= 10; nbint += (ch - '0'); break; }
