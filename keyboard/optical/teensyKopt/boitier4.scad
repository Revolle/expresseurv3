...
x2 = 0; // pour inclure un x2 à la place du s2
...
hporteled = hled + e ;
lporteled = lled + e ; // largeur porte led 7mm
...
module boite ()
{
    union()
    {
        difference()
        {
            ...
            union()
            {
                ...
                for(nrled = [-1:1] )
                {
                    translate([x_doigt/2 + x_electronique/2 -pouce ,nrled * iled,dzporteled + hled/2 - dhled ])
    ...
    if (composant == 1)
    {
        ...
        translate([-x_electronique/2 + xpcb/2 + pouce /2 ,0,dzpcb  ] ) 
            pcb() ;
        // pcbled
        translate([+x_electronique/2 + x_doigt + x_led/2 -e -pouce ,0 , dzpcbled  ] ) 
            pcbled() ;
        if ( pedale == 1 )
        {
