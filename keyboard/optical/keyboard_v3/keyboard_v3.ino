...
uint8_t resetPin[2] = { 2, 4 };  // digital pin for reset config button (one output, one input)
...
bool opticalProcess() {
  ...
  for (nr = 0, o = optical, modulo2 = true, allOff = true, opticalMeasure = false; nr < NB_OPTICAL; nr++, o++, modulo2 = !modulo2) {
    ...
    delay(200);
#endif
    if (tuning == TUNING_VALUE) {
      if (opticalSince > (DT_TUNING * 1000000 / 2)) {
        tuning = TUNING_SLOPE;
#if DEBUGMODE
        uint8_t nrl;
        T_optical *ol;
        prtln("definitive limits optical button #");
        for (nrl = 0, ol = optical; nrl < NB_OPTICAL; nrl++, ol++) {
          prt(ol->nr);
          prt(" ");
          prt(ol->v_min);
          prt(".. ");
          prt(ol->v_trigger_off);
          prt("=>");
          prt(ol->v_trigger_on);
          prt(".. ");
          prtln(ol->v_max);
        }
        prtln("tuning slope starts");
#endif
        ledOnboardFlash(20, true);
      } else {
...
///////////////
void resetInit() {
  pinMode(resetPin[0], OUTPUT);
  digitalWrite(resetPin[0], LOW);
